...
一旦某个bucket中数量超过8则触发overflow，通过extra字段对应的mapextra的overflow字段进行拓展，即申请一个新的bmap挂在这个bmap的后面形成链表，
...
```
由若干个bucket(即bmap)组成的数组，每个bucket存放不超过8个键值对。
...
#### 4、新增/修改
hmap指针传递的方式决定了map在使用前必须初始化，map也不支持并发读写。
```
runtime/map.go
...
map迭代是通过hiter结构和对应的两个辅助函数实现，hiter结构由编译器在调用辅助函数前创建并传入，每次迭代结果也由hiter结构传回。
* it.startBucket：开始的桶
* it.offset：桶中的偏移量
* it.bptr：当前遍历的桶
...
从hash数组中第it.startBucket个桶开始，先遍历hash桶和其溢出链表，之后hash数组偏移量+1，继续前一步动作。遍历每一个桶，无论是hash桶还是溢出桶，都从it.offset偏移量开始，
